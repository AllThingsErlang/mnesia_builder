...
-export([read/2, read_no_trans/2, select/4, select_or/6, select_and/6, build_matchhead/1]).
-export([write/1, write/2, write/3]).
-export([write_new/1, write_new/2, write_new/3]).
-export([write_exists/1, write_exists/2, write_exists/3]).
-export([write_if/3, write_if/4, write_if/5]).
-export([delete/1, delete/2, clear_all_tables/0]).
...
read(SchemaName, Key) -> mb_db_query:read(SchemaName, Key, get_ssg()).

read_no_trans(SchemaName, Key) -> mb_db_query:read_no_trans(SchemaName, Key, get_ssg()).
...
write_new(Record) -> mb_db_edit:write_new(Record, get_ssg()).

write_new(SchemaName, Record) -> mb_db_edit:write_new(SchemaName, Record, get_ssg()).

write_new(SchemaName, Key, Data) -> mb_db_edit:write_new(SchemaName, Key, Data, get_ssg()).

write_exists(Record) -> mb_db_edit:write_exists(Record, get_ssg()).

write_exists(SchemaName, Record) -> mb_db_edit:write_exists(SchemaName, Record, get_ssg()).

write_exists(SchemaName, Key, Data) -> mb_db_edit:write_exists(SchemaName, Key, Data, get_ssg()).

write_if(CheckFun, ArgList, Record) -> mb_db_edit:write_if(CheckFun, ArgList, Record, get_ssg()).

write_if(CheckFun, ArgList, SchemaName, Record) -> mb_db_edit:write_if(CheckFun, ArgList, SchemaName, Record, get_ssg()).

write_if(CheckFun, ArgList, SchemaName, Key, Data) -> mb_db_edit:write_if(CheckFun, ArgList, SchemaName, Key, Data, get_ssg()).
